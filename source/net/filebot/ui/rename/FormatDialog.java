...
public class FormatDialog extends JDialog {
	...
	private static final PreferencesEntry<String> persistentSampleFile = Settings.forPackage(FormatDialog.class)
			.entry("format.sample.file");
	...
	public FormatDialog(Window owner, Mode initMode, MediaBindingBean bindings, boolean locked) {
		...
		header.setBorder(new SeparatorBorder(1, javax.swing.UIManager.getColor("Component.borderColor"),
				javax.swing.UIManager.getColor("Component.borderColor"), GradientStyle.LEFT_TO_RIGHT, Position.BOTTOM));
		...
		content.add(createImageButton(selectFolderAction),
				"sg action, w 25!, h 19!, pos n editor.y2+2 editor.x2-(27*1) n");
		content.add(createImageButton(showRecentAction),
	...
	public void setState(Mode mode, MediaBindingBean bindings, boolean locked) {
		...
		if (locked) {
			...
			title.setText(String.format("%s ⇔ %s", mode, bindings.getInfoObject(),
		...
		setFormatCode(mode.persistentFormatHistory().isEmpty() ? mode.getDefaultFormatExpression()
	...
	private JComponent createSyntaxPanel(Mode mode) {
		...
		panel.setBorder(createLineBorder(javax.swing.UIManager.getColor("Component.borderColor")));
		panel.setBackground(javax.swing.UIManager.getColor("TextArea.background"));
		panel.setOpaque(true);

		panel.add(new LinkButton(newAction(
				ResourceBundle.getBundle(FormatDialog.class.getName()).getString(mode.key() + ".syntax"), evt -> {
	...
	private ExecutorService createExecutor() {
		...
		ThreadPoolExecutor executor = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.SECONDS,
				new LinkedBlockingQueue<Runnable>(1), new DefaultThreadFactory("PreviewFormatter"));
	...
	private void checkFormatInBackground() {
		try {
			...
			// activate delayed to avoid flickering when formatting takes only a couple of
			// milliseconds
				...
				protected void done() {
					try {
						...
						if (suppressed != null) {
							...
							if (bindingIssue != null && getMessage(bindingIssue)
			...
			// cancel old worker, after new worker has been created, because done() might be
			// called from within cancel()
	...
	protected final Action changeSampleAction = newAction("Change Sample", ResourceManager.getIcon("action.variables"),
			evt -> {
				BindingDialog dialog = new BindingDialog(getWindow(evt.getSource()), String.format("%s Bindings", mode),
						mode.getFormat(), !locked);
	...
	protected final Action selectFolderAction = newAction("Change Folder", ResourceManager.getIcon("action.load"),
			evt -> {
					...
					if (templatePath.isAbsolute()) {
						...
						for (File next : listPath(templatePath)) {
							if (existingPath != null && !next.exists()) {
								...
								relativeFormat = relativeFormat.substring(existingPath.getPath().length() + 1); // account
																												// for
																												// file
																												// separator
								break;
	...
	protected final Action showRecentAction = newAction("Change Format", ResourceManager.getIcon("action.menu"),
			evt -> {
	...
	protected final Action switchEditModeAction = newAction("Switch Mode", ResourceManager.getIcon("dialog.switch"),
			evt -> {
				Mode next = mode.next();
				setState(next, restoreSample(next), false);
			});

	protected final Action approveFormatAction = newAction("Use Format", ResourceManager.getIcon("dialog.continue"),
			evt -> {
				try {
					// check syntax
